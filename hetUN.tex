...
and $N$ flavours of quarks ($N$ hypermultiplets in the fundamental representation;
...
In Appendix~\ref{notations} we explain our notations.
...
the deformation of the \ntwot theory consists of adding a 2-dimensional superpotential
\[
...
\setcounter{equation}{0}
	...
	worldsheet theory is given by Eq.~\eqref{str_ntwot}.
...
We see that we have $N$ vacua  in the world sheet model, {\it i.e.} $N$ degenerate
elementary non-Abelian strings in the bulk theory . In fact, field $\sigma$  is 
proportional
to the bifermion condensate $\bar{\xi}_L\xi_R$ and its VEV signals of breaking of the 
chiral symmetry. In fact, classically the model has the U(1) chiral symmetry broken 
...
\]
which allows to conveniently treat them on the same footing with the gauge fields, when the gauge
...
We use euclidean space-time throughout the paper, both in the 4-dimensional and 2-dimensional
cases.
...
as the worldsheet (or ``longitudinal'') coordinates, and integrating over the orthogonal coordinates. 
One then identifies the lower-index spinors as the 2-dimensional left- and right-handed chiral spinors
\[
...
In the worldsheet theory, the CP($N-1$) indices are written as upper ones for the orientational variables
\[
	n^l\,, \quad \xi^l\,,
\]
