...
\notag
	...
	\Biggr\rgroup~.
\end{align}
	We have explicated the CP($N-1$) indices in the last term in \eqref{str_ntwot_cct} to avoid ambiguity.
...
\end{align}
	...
	in the fourth line of Eq.~\eqref{world02_unnorm}.
...
\setcounter{equation}{0}
	...
	Other than that we know the answer for rest of the effective worldsheet theory, given by Eq.~\eqref{str_ntwot_cct}.
...
\notag
	...
	~.
\end{align}
	Inspecting Eqs.~\eqref{N1_strans_eqn} and dropping terms proportional to $ O(\mu^2) $ one immediately observes that 
	the first-order profiles obey similar differential equations to those of the zero-order profiles, and thus are 
...
\subsection{Large-\boldmath{$\mu$} limit}
	...
	can be dropped from equations \eqref{N1_strans_eqn} and \eqref{N1_sorient_eqn}.
	...
	On the other hand, {\it e.g.} in Eqs.~\eqref{N1_strans_eqn}, it is functions $ f $ and $ f_N $ 
...
\subsubsection{Supertranslational zero-modes}
	...
	Dropping the kinetic terms for $ \lambda $'s in \eqref{N1_strans_eqn} and setting the string
...
\end{align}
	and substitute them back into \eqref{N1_strans_red},
\begin{align*}
%
...
\eeq
	...
	third equations in \eqref{N1_strans_eqn}.
	...
	Assuming this, and taking the fifth and seventh equations in \eqref{N1_strans_eqn}, one observes
...
\subsubsection{Superorientational zero-modes}
	...
	Resolving the gauginos from the first and the third equations of \eqref{N1_sorient_eqn},
\begin{align}
%
\notag
...
\end{align}
	and substituting them into the rest two equations in \eqref{N1_sorient_eqn}, while dropping 
...
\end{align}
	...
	We substitute the long-range tails \eqref{tail_strans} and \eqref{tail_sorient} into \eqref{norm_int},
\begin{align}
%
\notag
