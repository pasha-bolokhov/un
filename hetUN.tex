...
\begin{center}
{\bf P. A. Bolokhov$^{a}$, M.~Shifman$^{b}$, and \bf A.~Yung$^{b,c,d}$}
\end {center}
\vspace{0.3cm}
\begin{center}

$^a${\it Physics and Astronomy Department, University of Pittsburgh, Pittsburgh, Pennsylvania, USA}\\
...
188300, Russia}\\
$^d${\it Institute of Theoretical and Experimental Physics, Moscow
117259, Russia}
\end{center}
...
model directly from the U$(N)$ bulk theory. Parameters of the bulk theory are related to those of the world sheet theory. Qualitatively this relation is the same as in the
in U(2) case.
...
\cite{HT1,ABEKY,SYmon,HT2}, see also reviews \cite{Trev,SYrev,Jrev,Trev2}. 
...
breaking \ntwo down to \none, the situation drastically changes:
...
found. We do this job in the limit of small and large coefficients in front of the
...
of the physics of the \ntwoo CP$(N-1)$ model, while Sect.~9 presents our conclusions. In Appendix we explain our notations.
...
%which then is softly broken to $ \mc{N}=1 $. 
...
\]
	...
	refer to it now, see review \cite{SYrev} and \cite{SYhet} where this solution
is found in \none bulk theory for the case of U(2) gauge group.
...
%
	A_i^\text{SU($N$)} ~~=~~ \frac{1}{N}\, &\, U\, \lgr \begin{matrix}
							...
							0  & \ldots  & 1  &  0 \\
							0  & 0   & \ldots  &  - (N-1) 
					         \end{matrix} \rgr  \, U^{-1} (\p_i \alpha)\, f_{N}(r)  \\
%
...
\cite{SYmon} and used to determine fermionic superorientational zero modes
in \ntwo theory with U(2) gauge group. Here we make a U$(N)$ generalization of results
in \cite{SYmon}.
...
\nonumber\\[3mm]
\delta \nbar & = &  \sqrt{2}\,( \ov{\epsilon_R\, \xi_L}+ \ov{\epsilon_L\, \xi_R})
\nonumber\\[3mm]
...
\label{susy_ntwot}
\eeqn
where $\epsilon_R\equiv\epsilon_1$, $\epsilon_L\equiv\epsilon_2$ are parameters of 
two SUSY transformations and $\nabla_k=\p_k-iA_k$, while the gauge potential is given by
\beqn
...
\section{Physics of the heterotic  CP$(N-1)$ model}
\setcounter{equation}{0}
\label{largeN}

In this section we briefly review physics of the heterotic \ntwoo  CP$(N-1)$
model which as we showed above is an effective low energy theory on the world sheet 
of the non-Abelian string in the deformed \ntwo QCD.
 \ntwoo  CP$(N-1)$ model was solved in \cite{SYhetlN} in the  large $N$ approximation.
This method was suggested by Witten and used to solve non-supersymmetric
and \ntwot supersymmetric CP$(N-1)$ model \cite{W79}. Results obtained in 
\cite{SYhetlN} show that the  \ntwoo supersymmetry is in fact spontaneously broken
( see also \cite{Tohetdyn}) and the vacuum energy density is non-zero. This means
that  tensions of elementary strings are lifted from their classical BPS values,
\beq
T=2\pi\xi +\frac{N}{4\pi}\,\Lambda_{\sigma}(1-e^{-u}),
\label{vacenergy}
\eeq
where the deformation parameter $u$ is related to $\delta$ as 
\beq
u\equiv \frac{16\pi\beta}{N}\,|\delta|^2.
\label{u}
\eeq
At large $\mu$ parameter $u$ behavies as 
\beq
u\sim\ln{\frac{g_2^2\mu}{m_W}}
\eeq

Like its \ntwot cousine the hetreotic \ntwoo model has $N$ degenerate vacua.
They are labeled by the non-zero VEV of the complex scalar $\sigma$ (former \ntwot
...
We see that we have $N$ vacua  in the world sheet model i.e. $N$ degenerative 
elementary non-Abelian strings in the bulk theory . In fact, field $\sigma$  is 
proportional
to the bifermion condensate $\bar{\xi}_L\xi_R$ and its VEV means breaking of the 
chiral symmetry. In fact, classically the model has chiral U(1) symmetry broken 
by the chiral anomaly down to the discrete $Z_{2N}$ symmetry. The non-zero VEV
of $\sigma$  breaks this $Z_{2N}$ symmetry further down to $Z_2$.

As soon as we have $N$ vacua in the model we also have kinks interpolating between
these vacua. From the point of view of the bulk theory they are interpreted as confined 
monopoles \cite{T,SYmon,HT2,SYnone,GSYmmodel,SYhetlN}. These monopoles are free to move
along the string. This should be contrasted to the non-supersymmetric CP$(N-1)$ model where
kinks  and anti-kinks form bound states, kink-anti-kink
``mesons'' \cite{W79}. From the four dimensional point of view this means that
monopoles (besides the four dimensional confinement) are in the phase of the two dimensional
confinement which ensures that monopole and anti-monopole  form a 
meson-like bound state on the string \cite{GSY05}.

The presence of $N$ degenerate vacua (\ref{sigmavev}) shows that monopoles
are deconfined (in the two dimensional sense) on the string in the theory at hand.

	When $ \mu $ is taken to infinity $N$ vacua (\ref{sigmavev}) coalesce and 
the worldsheet theory supposedly flows into
 a conformal phase \cite{SYhetlN}.
	However, this theory cannot be safely trusted in this limit, since the presence
	of massless modes \eqref{light} makes the string swell, and higher-order 
corrections to
...
namely direct derivation of world sheet theory for heterotic non-Abelian strings
...
supersymmetric QCD, now we we analyzed U$(N)$ with arbitrary $N$.
